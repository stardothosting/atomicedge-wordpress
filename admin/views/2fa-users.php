...
?>
<div class="wrap">
	<h1><?php echo esc_html__( '2FA User Management', 'atomicedge' ); ?></h1>
	
	<p class="description">
		<?php echo esc_html__( 'View and manage Two-Factor Authentication status for all users.', 'atomicedge' ); ?>
	</p>
	...
	<div class="atomicedge-stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; margin: 20px 0; max-width: 600px;">
		<div class="atomicedge-stat-card" style="background: #fff; padding: 20px; border-left: 4px solid #0073aa; box-shadow: 0 1px 1px rgba(0,0,0,.04);">
			...
			<div class="stat-label" style="color: #666; font-size: 13px;"><?php echo esc_html__( 'Total Users', 'atomicedge' ); ?></div>
		</div>
		<div class="atomicedge-stat-card" style="background: #fff; padding: 20px; border-left: 4px solid #46b450; box-shadow: 0 1px 1px rgba(0,0,0,.04);">
			...
			<div class="stat-label" style="color: #666; font-size: 13px;"><?php echo esc_html__( '2FA Enabled', 'atomicedge' ); ?></div>
		</div>
		<div class="atomicedge-stat-card" style="background: #fff; padding: 20px; border-left: 4px solid #f0ad4e; box-shadow: 0 1px 1px rgba(0,0,0,.04);">
			...
			<div class="stat-label" style="color: #666; font-size: 13px;"><?php echo esc_html__( 'No 2FA', 'atomicedge' ); ?></div>
		</div>
	</div>
	...
	<table class="wp-list-table widefat fixed striped users">
		<thead>
			<tr>
				...
				<th scope="col" class="manage-column" style="width: 120px;"><?php echo esc_html__( 'Role', 'atomicedge' ); ?></th>
				<th scope="col" class="manage-column" style="width: 120px;"><?php echo esc_html__( '2FA Status', 'atomicedge' ); ?></th>
				<th scope="col" class="manage-column" style="width: 120px;"><?php echo esc_html__( 'Actions', 'atomicedge' ); ?></th>
			</tr>
		</thead>
		<tbody>
			...
			<?php else : ?>
				<?php foreach ( $filtered_users as $user ) : ?>
					...
					?>
					<tr>
						<td class="column-username column-primary" data-colname="<?php echo esc_attr__( 'Username', 'atomicedge' ); ?>">
							<strong>
						...
						<td data-colname="<?php echo esc_attr__( 'Name', 'atomicedge' ); ?>">
							<?php echo esc_html( $user->display_name ); ?>
						</td>
						<td data-colname="<?php echo esc_attr__( 'Email', 'atomicedge' ); ?>">
							<a href="mailto:<?php echo esc_attr( $user->user_email ); ?>">
								<?php echo esc_html( $user->user_email ); ?>
							</a>
						</td>
						<td data-colname="<?php echo esc_attr__( 'Role', 'atomicedge' ); ?>">
							<?php
						...
						<td data-colname="<?php echo esc_attr__( '2FA Status', 'atomicedge' ); ?>">
							<?php if ( $has_2fa ) : ?>
									...
									<small style="color: #dc3232;">
										...
										if ( 'grace_period' === $policy_status['status'] ) {
											printf(
												...
												esc_html( _n( '%d day to set up', '%d days to set up', $policy_status['days_remaining'], 'atomicedge' ) ),
												esc_html( $policy_status['days_remaining'] )
											);
										} else {
											echo esc_html__( 'Required by policy', 'atomicedge' );
										}
										?>
									</small>
								<?php endif; ?>
						...
						<td data-colname="<?php echo esc_attr__( 'Actions', 'atomicedge' ); ?>">
							<?php if ( $has_2fa ) : ?>
